...
def create_trainfile(flow):
    ...
    if flow==1:                                         # os.getcwd is used if the py file is used to be called from another main file 
      ...
      files1 = os.path.abspath('../dataset/Test/docsets/all_tweets_test.txt')     # Here when we  all this py file docset.txt should be absolute path                 
      files = ['../dataset/Test/liblinear/all_tweets_test.txt']                          #add training files when necessary
...
def testSVM():
    m=svm_load_model('../dataset/Trained/models/modelsn08and03/sports.model')
    labelsS,featuresS=svm_read_problem('../dataset/Test/liblinear/'+inp)
    s_label, p_acc, p_val = svm_predict(labelsS, featuresS, m)
    print "Sports:  "+str(s_label)
    c = Counter(s_label)
    print c   

    m1=svm_load_model('../dataset/Trained/models/modelsn08and03/finance.model')
    ...
    m2=svm_load_model('../dataset/Trained/models/modelsn08and03/politics.model')
    ...
    m3=svm_load_model('../dataset/Trained/models/modelsn08and03/technology.model')
    labelsP,featuresP=svm_read_problem('../dataset/Test/liblinear/'+inp)
    ...
    m4 = svm_load_model('../dataset/Trained/models/modelsn08and03/entertainment.model')
    labelsP, featuresP = svm_read_problem('../dataset/Test/liblinear/'+inp)
